...
public class VentasController {
    ...
    @Operation(summary = ApiConstants.VentasControllerConstants.FIND_ALL_ITEMS_URL_SUMMARY,
    ...
    @GetMapping(ApiConstants.VentasControllerConstants.FIND_ITEM_BY_BARCODE_ADDRESS_URL)
    @Operation(summary = ApiConstants.VentasControllerConstants.FIND_ITEM_BY_BARCODE_ADDRESS_URL_SUMMARY,
            description = ApiConstants.VentasControllerConstants.FIND_ITEM_BY_BARCODE_ADDRESS_URL_DESCRIPTION)
    @ApiResponse(content = {
        @Content(mediaType = ApiConstants.CONTENT_TYPE_JSON_APPLICATION)})
    public ResponseEntity<Item> findItemByBarcode(@PathVariable("barcode") String barcode) {
        ...
        if (item == null) {
            ...
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "No exite el item con código: " + barcode);
        }
